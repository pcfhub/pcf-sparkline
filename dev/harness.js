...
(function () {
    ...
    function options() {
        ...
        return {
            ...
            pageSize: Number(document.getElementById('harness-pagesize').value) || 100,
            ...
            /*
             * The named series in fixture.js — one reading, a flat series,
             * nothing readable, nothing at all — carry only what differs, so a
             * missing key means "as the main fixture has it".
             */
            records: state === 'loading' ? [] : named && named.records ? named.records : null,

            /*
             * "The value column was never mapped" is a real canvas state: the
             * maker picked columns in the Items Fields flyout and has not
             * finished. An empty box reads as a broken control rather than as
             * an unfinished configuration.
             */
            columns: named && named.columns ? named.columns : null,

            // Every declared input, as the platform always supplies them.
            inputs: {
                chartType: document.getElementById('harness-charttype').value,
                chartHeight: Number(document.getElementById('harness-chartheight').value) || 96,
                allowFullScreen: document.getElementById('harness-allowfullscreen').checked,
            },
        ...
        document.getElementById('harness-calls').textContent =
            handle.state.calls.length > 0
                ? handle.state.calls
                    ...
                    .join('\n')
                : 'Nothing yet. Click Expand, or change the records to chart.';
    }
